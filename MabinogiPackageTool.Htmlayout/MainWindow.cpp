...
namespace htmlayout
{
	...
	HINSTANCE MainWindow::hinstance = 0;

	const wchar_t CLASSNAME[] = L"htmlayout::MainWindow";

	// register window class

	ATOM  MainWindow::register_class(HINSTANCE hInstance)
	{
		hinstance = hInstance;
		...
		wcex.hIcon = LoadIcon(hInstance, (LPCTSTR)IDI_MABINOGIPACKAGETOOLHTMLAYOUT);
...
#define WS_EX_LAYERED           0x00080000
	...
	MainWindow* MainWindow::create(int x, int y, int width, int height, const wchar_t* caption)
	{
		...
		if (load_resource_data(L"DEFAULT", pb, cb))
		{
			...
			pw->topbar = r.get_element_by_id("topbar");
	...
	void MainWindow::set_caption(const wchar_t* text)
	{
		if (text)
		{
	...
	HELEMENT  MainWindow::root()
	{
		return dom::element::root_element(hwnd);
	}

	int MainWindow::hit_test(int x, int y)
	{
		...
		/*if (topbar.is_valid() && topbar.is_inside(pt))
		{
			return HTCAPTION;
		}*/
	...
	LRESULT CALLBACK MainWindow::win_proc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
	{
		LRESULT lResult;
		BOOL    bHandled;
